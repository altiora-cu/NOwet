...
export function CardPack({ pack }: { pack: Pack }) {
  ...
  return (
    <div className="relative flex h-full w-full flex-col overflow-hidden rounded-md border border-line bg-surface transition-all duration-hover ease-out hover:-translate-y-1 hover:shadow-md motion-reduce:hover:translate-y-0">
      ...
      <Link href={`/packs#${pack.slug}`} className="relative block aspect-[6/5] bg-surface-alt">
        <Image src={pack.imagen} alt={pack.imagenAlt} fill sizes="288px" className="object-contain p-4" />
      </Link>
